...
public class AnnotationsAPIController {
	...
	@RequestMapping(value = "/europePMCannotations", method = RequestMethod.GET)
	public ResultEuropePMC getAnnotationsArticlesEuropePMC(@RequestParam(value="artId") String artId, @RequestParam(value="abstractOnly") boolean abstractOnly,
												 @RequestParam(value="provider", defaultValue=DEFAULT_FILTER_PARAM_VALUE) AnnotationProvider provider) throws Exception {
		...
		List<String> artIds = Arrays.asList(artId);
		validateArticleIds(artIds, 1);
		
		ResultEuropePMC ret = new ResultEuropePMC();
		
		String [] artIdInfo= artId.split(":");
		
		String src= artIdInfo[0];
		
		String extId= artIdInfo[1];
		
		Query query;
		boolean prePrintsfulltext=false;
		if ("PMC".equalsIgnoreCase(src)){
			Long pmcid = Long.parseLong(extId.replaceAll("PMC", ""));
			query = DBQuery.is("pmcid", pmcid);
			if (pmcid<0 && (abstractOnly==false) && provider==AnnotationProvider.EUROPEPMC){
				prePrintsfulltext=true;
			}
		}else{
			query = DBQuery.and(DBQuery.is("src", src), DBQuery.is("ext_id", extId));
		}
		...
		AnnotationApi articlesData=null;
		AnnotationMultiple preprintsData=null;
		
		if (prePrintsfulltext==false){
			articlesData = dao.findEntry(collectionName, AnnotationApi.class, query);
		}else{
			preprintsData = dao.findEntry("annotationsMultiple", AnnotationMultiple.class, query);
		}
		
		if (articlesData!=null || preprintsData!=null){
			if (prePrintsfulltext==false){
				List<AnnotationItemApi> filteredAnnotations = articlesData.getAnns().stream().filter(AnnotationsLambda.filterAnnotationsEuropePMC(abstractOnly, provider.getDbValue())).collect(Collectors.<AnnotationItemApi>toList());
				articlesData.setAnns(filteredAnnotations);
				ret = new ResultEuropePMC(articlesData, provider.getDbValue());
			}else{
				ret = new ResultEuropePMC(preprintsData, provider.getDbValue());
			}
			
		}else{
			ret.setProvider(provider.getDbValue());
			ret.setSrc(src);
			ret.setExtId(extId);
		}
		
		long end = System.currentTimeMillis();
		logger.info( "Retrieved data for EuropepMC articles for (src:ext_id)  ("+artId+") in "+ (end - start)+ " ms");
		
		return ret;
	}
	
	
	@RequestMapping(value = "/europePMCannotationsLinkBack", method = RequestMethod.GET)
	public
	@ResponseBody
	ResultEuropePMC getAnnotationsArticlesEuropePMCLinkBack(@RequestParam(value="artId") String artId, @RequestParam(value="abstractOnly") boolean abstractOnly,
			 @RequestParam(value="annotationId") String annotationId)throws Exception {
		
		ResultEuropePMC ret = new ResultEuropePMC();
		
		String linkBackPrefix=annotationId.split("-")[0];
		AnnotationProviderApi providerType = AnnotationProviderApi.getProviderTypeByLinkback(linkBackPrefix);
		
		if (providerType==AnnotationProviderApi.UNKNOWN){
			throw new AnnotationsAPIParameterException(AnnotationProvider.getValidationMessage());
		}
		...
		Long pmcid=null;
		if ("PMC".equalsIgnoreCase(src)){
			pmcid = Long.parseLong(extId.replaceAll("PMC", ""));
			query = DBQuery.is("pmcid", pmcid);
		}else{
			query = DBQuery.and(DBQuery.is("src", src), DBQuery.is("ext_id", extId));
		}
		...
		AnnotationApi articlesData = dao.findEntry(collectionName, AnnotationApi.class, query);
		
		if (articlesData!=null){
			String linkBack= AnnotationItemApi.getLinkBack(src, extId, pmcid, abstractOnly, annotationId);
			
			List<AnnotationItemApi> filteredAnnotations = articlesData.getAnns().stream().filter(AnnotationsLambda.filterAnnotationsEuropePMCLinkBack(abstractOnly, providerType.getLabel(), linkBack)).collect(Collectors.<AnnotationItemApi>toList());
			articlesData.setAnns(filteredAnnotations);
			ret = new ResultEuropePMC(articlesData, providerType.getLabel());
			
		}else{
			ret.setProvider(providerType.getLabel());
			ret.setSrc(src);
			ret.setExtId(extId);
		}
	...
	public List<Object> getAnnotationsArticlesByIds(@ApiParam(value="Article Id lists contain between 1 and 8 values. Each value must follow the format SOURCE:EXTERNAL_ID. Allowed values for the field SOURCE are <i>MED</i>: PubMed MEDLINE, <i>PMC</i>: PubMedCentral not in (PubMed),"+
			                                     ...
			                                     @ApiParam(value="Provider of the annotations that the user is interested in. If this parameter is specified only annotations provided from the specified provider will be retrieved", allowableValues="Europe PMC,HES-SO_SIB,OpenTargets,NaCTeM,IntAct,DisGeNET,NCBI_PubTator,BSC/NTNU", required=false) @RequestParam(value="provider", defaultValue=DEFAULT_FILTER_PARAM_VALUE) AnnotationProvider provider, 
	...
	public Result getAnnotationsArticlesByProvider(	@ApiParam(value="Provider of the annotations that the user is interested in.", allowableValues="Europe PMC,HES-SO_SIB,OpenTargets,NaCTeM,IntAct,DisGeNET,NCBI_PubTator,BSC/NTNU", required=true) @RequestParam(value="provider") AnnotationProvider provider, 
	...
	public Result getAnnotationsArticlesBySectionAndOrType(@ApiParam(value="Type of the annotations that the user is interested in", allowableValues="Gene_Proteins,Organisms,Chemicals, Gene Ontology,Diseases,Accession Numbers,Gene Function,Gene Disease,Protein Interaction,Biological Event,Gene Mutations,TF_TG" ) @RequestParam(value="type" , defaultValue=DEFAULT_FILTER_PARAM_VALUE) AnnotationType type, 
